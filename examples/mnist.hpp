...
#ifndef PURINE_MNIST_
#define PURINE_MNIST_
...
class Mnist: public Graph {
    protected:
        ...
        int batch_size;
    public:
        explicit Mnist(int rank, int device, int bs);
        virtual ~Mnist() override {}
    ...
    template <bool test>
Mnist<test>::Mnist(int rank, int device, int bs)
    : Graph(rank, device) {
        batch_size = bs;

        data_ = create("data", { batch_size, 1, 28, 28 });
        data_diff_ = create("data_diff", { batch_size, 1, 28, 28 });
